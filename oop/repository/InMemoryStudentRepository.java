...
// Hiện thực lưu trữ bằng bộ nhớ (Map)
public class InMemoryStudentRepository implements StudentRepository {
    private final Map<Integer, Student> data = new LinkedHashMap<>();

    @Override public Student save(Student s) { data.put(s.getId(), s); return s; }
    @Override public Optional<Student> findById(int id) { return Optional.ofNullable(data.get(id)); }
    @Override public List<Student> findAll() { return new ArrayList<>(data.values()); }
    @Override public boolean deleteById(int id) { return data.remove(id) != null; }
